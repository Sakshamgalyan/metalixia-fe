...
const Chart = ({
  ...
  const h =
    typeof height === 'number' ? height : parseInt(height as string) || 200;
  const w = width
    ? typeof width === 'number'
      ? width
  ...
  if (type === 'line') {
    ...
    const points = useMemo(
      () =>
        safeData.map((v, i) => ({
          x:
            pad.left +
            (i / Math.max(safeData.length - 1, 1)) *
              (vW - pad.left - pad.right),
    ...
    const gridY = showGridLines
      ? [0.25, 0.5, 0.75].map((f) => pad.top + f * (vH - pad.top - pad.bottom))
      : [];
    ...
    return (
      <div className={`w-full flex flex-col ${className}`}>
        ...
        {showXAxis && labels && (
          <motion.div
            className="flex justify-between mt-3 text-[10px] font-semibold text-slate-400 uppercase tracking-[0.12em] px-1 select-none"
            initial={animated ? { opacity: 0, y: 4 } : undefined}
            animate={animated ? { opacity: 1, y: 0 } : undefined}
            transition={{ delay: 0.6, duration: 0.4 }}
          >
            {labels.map((l, i) => (
              <span key={i} className="transition-colors hover:text-slate-600">
                {l}
              </span>
            ))}
          </motion.div>
        ...
        <div style={{ height: `${h}px`, width: `${h}px` }} className="relative">
          <svg viewBox="0 0 100 100" className="w-full h-full overflow-visible">
            ...
            {slices.map((s, i) => (
              <motion.path
                ...
                transition={
                  animated
                    ? { delay: i * 0.06, duration: 0.5, ease: 'backOut' }
                    : { duration: 0.2 }
                }
            ...
            {slices
              ...
              .map((s, i) => (
                <text
                  ...
                  className="fill-white text-[5px] font-bold pointer-events-none select-none"
            ...
            {ir > 0 && (
              <text
                ...
                textAnchor="middle"
                dominantBaseline="central"
                className="fill-slate-700 text-[8px] font-black pointer-events-none select-none"
              >
                {total}
              </text>
            )}
          ...
          {showTooltip && hovered !== null && (
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-[calc(100%+20px)] bg-slate-900/90 backdrop-blur-md text-white text-[11px] py-1.5 px-3 rounded-lg shadow-xl border border-white/10 whitespace-nowrap font-medium z-30 pointer-events-none">
              ...
              >
                {labels?.[hovered] || `Slice ${hovered + 1}`}
              </span>
              <span className="text-slate-300 ml-1.5">
                {safeData[hovered]} (
                {Math.round((safeData[hovered] / total) * 100)}%)
              </span>
            </div>
          )}
        </div>
            ...
            {labels.map((l, i) => (
              ...
              >
                <div
                  ...
                  style={{ backgroundColor: getColor(i, colors) }}
                />
                <span>{l}</span>
              </div>
            ))}
            ...
            return (
              ...
              >
                <div
                  ...
                  style={{ backgroundColor: getColor(i, colors || [color]) }}
                />
        ...
        <div className="relative w-full" style={{ height: `${h}px` }}>
          ...
          >
            ...
            {scatterData.map((pt, i) => {
              const px =
                pad + ((pt.x - minX) / (maxX - minX || 1)) * (vW - 2 * pad);
              const py =
                vH -
                pad -
                ((pt.y - minY) / (maxY - minY || 1)) * (vH - 2 * pad);
              return (
                <motion.circle
                  key={i}
                  cx={px}
          ...
          {showTooltip && hovered !== null && scatterData[hovered] && (
            <div className="absolute top-2 right-2 bg-slate-900/90 backdrop-blur-md text-white text-[11px] py-1.5 px-3 rounded-lg shadow-xl border border-white/10 whitespace-nowrap font-medium z-30 pointer-events-none">
              <span className="font-bold" style={{ color }}>
                {scatterData[hovered].label || `Point ${hovered + 1}`}
              </span>
              <span className="text-slate-300 ml-1.5">
                ({scatterData[hovered].x}, {scatterData[hovered].y})
              </span>
            </div>
          )}
        </div>
        ...
        <div className="relative w-full" style={{ height: `${h}px` }}>
              ...
              return (
                <React.Fragment key={i}>
                  ...
                  <motion.circle
                    ...
                    className="cursor-pointer"
                    initial={animated ? { scale: 0, opacity: 0 } : undefined}
                    animate={{ scale: 1, opacity: hovered === i ? 0.95 : 0.7 }}
                    transition={{
                      delay: i * 0.04,
                      duration: 0.4,
                      ease: 'backOut',
          ...
          {showTooltip && hovered !== null && bubbleData[hovered] && (
            <div className="absolute top-2 right-2 bg-slate-900/90 backdrop-blur-md text-white text-[11px] py-1.5 px-3 rounded-lg shadow-xl border border-white/10 whitespace-nowrap font-medium z-30 pointer-events-none">
              <span
                className="font-bold"
                style={{ color: getColor(hovered, colors) }}
              >
                {bubbleData[hovered].label || `Bubble ${hovered + 1}`}
              </span>
              <span className="text-slate-300 ml-1.5">
                ({bubbleData[hovered].x}, {bubbleData[hovered].y}) r=
                {bubbleData[hovered].r}
              </span>
            </div>
          )}
        </div>
  ...
  if (type === 'boxplot' && boxPlotData) {
    ...
    const boxW = Math.min(
      12,
      (vW - pad.left - pad.right) / boxPlotData.length - 2,
    );

    const mapY = (v: number) =>
      vH -
      pad.bottom -
      ((v - globalMin) / (globalMax - globalMin || 1)) *
        ...
        <div className="relative w-full" style={{ height: `${h}px` }}>
          ...
          >
            ...
            {boxPlotData.map((b, i) => {
              const cx =
                pad.left +
                (i + 0.5) * ((vW - pad.left - pad.right) / boxPlotData.length);
              const c = getColor(i, colors);
              return (
                <motion.g
                  key={i}
                ...
                >
                  ...
                  <rect
                    ...
                    y={mapY(b.q3)}
                    width={boxW}
                    height={mapY(b.q1) - mapY(b.q3)}
                    fill={c}
                    fillOpacity={hovered === i ? 0.3 : 0.15}
                    stroke={c}
                    strokeWidth="0.8"
                    rx="1"
                  />
          ...
          {showTooltip && hovered !== null && boxPlotData[hovered] && (
            <div className="absolute top-2 right-2 bg-slate-900/90 backdrop-blur-md text-white text-[10px] py-2 px-3 rounded-lg shadow-xl border border-white/10 whitespace-nowrap font-medium z-30 pointer-events-none leading-relaxed">
              <div
                ...
                style={{ color: getColor(hovered, colors) }}
              >
                {boxPlotData[hovered].label || `Box ${hovered + 1}`}
              </div>
              <div>
                Max: {boxPlotData[hovered].max} · Q3: {boxPlotData[hovered].q3}
              </div>
              <div>Med: {boxPlotData[hovered].median}</div>
              <div>
                Q1: {boxPlotData[hovered].q1} · Min: {boxPlotData[hovered].min}
              </div>
            </div>
          )}
        </div>
            ...
            {rows.map((row, ri) => (
              <React.Fragment key={ri}>
                <text
                  ...
                  textAnchor="end"
                  dominantBaseline="central"
                  className="fill-slate-400 text-[3px] font-semibold uppercase select-none"
                >
                  {row}
                </text>
                {cols.map((col, ci) => {
  ...
  if (type === 'gantt' && ganttData) {
    ...
    return (
      <div className={`w-full flex flex-col ${className}`}>
        <div className="relative w-full" style={{ height: `${h}px` }}>
          <svg
            viewBox={`0 0 100 ${5 + ganttData.length * (rowH + 2)}`}
            className="w-full h-full overflow-visible"
            preserveAspectRatio="xMinYMin meet"
          >
            {ganttData.map((g, i) => {
              ...
              return (
                <React.Fragment key={i}>
                  ...
                  <text
                    ...
                    textAnchor="end"
                    dominantBaseline="central"
                    className="fill-slate-500 text-[3px] font-semibold select-none"
                  >
                    {g.label}
                  </text>
                  {/* Row background */}
          ...
          {showTooltip && hovered !== null && ganttData[hovered] && (
            <div className="absolute top-2 right-2 bg-slate-900/90 backdrop-blur-md text-white text-[11px] py-1.5 px-3 rounded-lg shadow-xl border border-white/10 whitespace-nowrap font-medium z-30 pointer-events-none">
              <span
                className="font-bold"
                style={{
                  color: ganttData[hovered].color || getColor(hovered, colors),
                }}
              >
                {ganttData[hovered].label}
              </span>
              <span className="text-slate-300 ml-1.5">
                {ganttData[hovered].start} → {ganttData[hovered].end}
              </span>
            </div>
          )}
        </div>
  ...
  return (
    <div className={`w-full flex flex-col ${className}`}>
      <div
        className="flex items-end justify-between gap-[3px] w-full relative"
        style={{ height: `${h}px` }}
      >
        {showGridLines && (
          <div className="absolute inset-0 flex flex-col justify-between pointer-events-none py-2">
        ...
        {safeData.map((value, index) => {
          ...
          return (
            <div
              key={index}
              className="flex-1 relative group cursor-pointer h-full flex items-end overflow-visible max-w-[10px] mx-auto"
            ...
            >
              ...
              >
                <div
                  className={`absolute inset-0 bg-gradient-to-t from-transparent via-white/25 to-transparent transition-transform duration-700 ease-in-out ${isHovered ? '-translate-y-full' : 'translate-y-full'}`}
                />
                <div
                  className={`absolute top-0 left-1/2 -translate-x-1/2 w-[6px] h-[6px] rounded-full bg-white/50 blur-[2px] transition-opacity duration-300 ${isHovered ? 'opacity-100' : 'opacity-0'}`}
                />
              </motion.div>
              {showTooltip && (
                <div
                  className={`absolute -top-14 left-1/2 -translate-x-1/2 z-30 pointer-events-none transition-all duration-300 ease-out ${isHovered ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 translate-y-2 scale-95'}`}
                >
      ...
      {showXAxis && labels && (
        ...
        >
          {labels.map((l, i) => (
            <span
              key={i}
              className={`transition-colors cursor-default ${hovered === i ? 'text-slate-700' : 'hover:text-slate-500'}`}
            >
              {l}
            </span>
          ))}
