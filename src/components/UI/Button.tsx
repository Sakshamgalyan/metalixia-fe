...
const Button = ({
  ...
  const variantClasses = {
    primary:
      'bg-gradient-to-r from-[#707FDD] to-[#5a67c4] hover:from-[#5a67c4] hover:to-[#4a57b4] text-white shadow-md hover:shadow-lg',
    secondary:
      'bg-slate-100 hover:bg-slate-200 border-2 border-slate-200 hover:border-slate-300',
    outline:
      'bg-transparent hover:bg-slate-50 border-2 border-slate-300 hover:border-slate-400',
    ...
    tertiary:
      'bg-transparent hover:bg-slate-50 text-slate-600 font-medium justify-start hover:text-slate-900 border border-transparent',
  };
  ...
  return (
    ...
    >
      ...
      {!isLoading && leftIcon && (
        <span
          className={`${iconSizes[size]} flex-shrink-0 flex items-center justify-center`}
        >
          {leftIcon}
        </span>
      )}
