...
const PROCESS_SHORT = [
  "DEG", "RIN", "ACD", "RIN", "ZNC", "RIN", "PAS", "RIN", "DRY", "HTR", "INS", "CLN",
];

const priorityColors: Record<string, "danger" | "warning" | "success" | "default" | "primary"> = {
  urgent: "danger",
  high: "warning",
  normal: "primary",
  low: "default",
  ...
  const columns: TableColumn<ProductionOrder>[] = [
    {
      ...
      render: (row) => (
        <div className="flex flex-col">
          <span className="font-mono font-bold text-slate-900 text-sm">{row.batchId}</span>
      ...
      render: (row) => (
        <div className="flex items-center gap-[2px]">
          {row.processes.map((proc, idx) => {
            ...
            if (proc.status === "in_progress") bg = "bg-amber-500 animate-pulse";
            return (
              <div
                key={idx}
    ...
    },
    {
      ...
      render: (row) => {
        if (row.status === "queued") {
          return (
            <Button size="sm" bgColor="#059669" textColor="#fff" leftIcon={<PlayCircle size={14} />} onClick={() => handleStartProduction(row._id)}>
              Start
            </Button>
          );
        }
        if (row.status === "in_production") {
          ...
          return (
            <Button
              ...
              textColor="#fff"
              leftIcon={lastProc ? <Zap size={14} /> : <ChevronRight size={14} />}
              onClick={() => lastProc ? handleSendToQC(row._id) : handleAdvance(row._id)}
            >
    ...
    <div className="space-y-6 w-[95%] mx-auto py-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
          <Typography variant="h5" className="mb-4">Throughput (Last 7 Days)</Typography>
          <Chart
        ...
        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
          <Typography variant="h5" className="mb-4">Process Pipeline</Typography>
          <div className="space-y-2">
              ...
              return (
                <div key={proc.index}>
                  <div className="flex justify-between text-xs mb-1">
                    <span className="text-slate-600 truncate max-w-[140px]">{proc.name}</span>
                    <span className="font-bold text-slate-800">{proc.count}</span>
                  </div>
            ...
            {(!statsData?.processCounts || statsData.processCounts.length === 0) && (
              <p className="text-sm text-slate-400 text-center py-6">No active production</p>
            )}
          </div>
        </div>
      </div>
