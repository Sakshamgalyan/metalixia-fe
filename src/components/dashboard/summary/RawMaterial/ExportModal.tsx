...
const ExportModal = () => {
  ...
  const handleExportCSV = () => {
    ...
    try {
      ...
      link.setAttribute(
        'download',
        `RawMaterials_Export_${new Date().toLocaleDateString()}.csv`,
      );
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} title="Export Data" width="md">
        ...
        <div className="flex items-start gap-4 p-4 bg-blue-50 border border-blue-100 rounded-lg text-blue-800">
          ...
          <div>
            ...
            <Typography variant="p" className="text-xs text-blue-600 mt-1">
              This will export the currently loaded {listData?.data.length || 0}{' '}
              entries into your selected format.
            </Typography>
          </div>
        </div>
