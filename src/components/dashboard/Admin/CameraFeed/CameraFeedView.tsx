...
const CameraFeedView = () => {
  ...
  const handleSaveConfig = () => {
    setCameras((prev) =>
      prev.map((c) =>
        c.id === editingCamId ? ({ ...c, ...editForm } as Camera) : c,
      ),
    );
  ...
  const handlePTZ = async (action: string) => {
    ...
    try {
      ...
      if (cam.username && cam.password) {
        headers.set(
          'Authorization',
          'Basic ' + btoa(cam.username + ':' + cam.password),
        );
      }
  ...
  return (
    <div className="h-[calc(100vh-100px)] flex flex-col p-6 max-w-[1600px] mx-auto gap-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-slate-200 pb-4">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">
            ...
            {cameras.filter((c) => c.status === 'Live').length}/{cameras.length}{' '}
            Cameras Online
          </p>
        </div>
      ...
      <div className="flex-1 grid grid-cols-12 gap-6 min-h-0">
        {/* Camera Grid */}
        <div
          className={`col-span-12 lg:col-span-9 grid gap-4 h-[600px] lg:h-full auto-rows-fr ${getGridClass()}`}
        >
        ...
        footer={
          <>
            ...
            <Button
              ...
              className="bg-[#707FDD] hover:bg-[#5b6ac0]"
            >
              Save Changes
            </Button>
          </>
          ...
          <div className="border-t border-slate-200 pt-5 mt-2">
            ...
            <div className="mb-4">
              <Input
                ...
                className="mb-1"
              />
              <p className="text-xs relative -top-1 px-1 text-slate-500">
                For Hikvision cameras, use the HTTP ISAPI preview stream which
                directly renders in browser grids.
              </p>
            </div>
            ...
            <Input
              ...
              onChange={(e) =>
                setEditForm((prev) => ({ ...prev, ptzUrl: e.target.value }))
              }
            ...
            <p className="text-xs text-slate-400 mt-2">
              Credentials will be encoded and passed as Basic Authorization
              headers for authorized PTZ queries from the browser.
            </p>
          </div>
        </div>
      </Modal>
