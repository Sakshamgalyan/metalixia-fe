...
export const ProductionStateContext = createContext<StateContext | undefined>(
  undefined,
);
export const ProductionDispatchContext = createContext<
  DispatchContext | undefined
>(undefined);

const ProductionContextProvider: React.FC<{ children: ReactNode }> = ({
  children,
}) => {
  const [state, dispatch] = useReducer(reducer, initialState);
  const enhancedDispatch = useCallback(
    (action: ProductionAction) => dispatch(action),
    [dispatch],
  );
