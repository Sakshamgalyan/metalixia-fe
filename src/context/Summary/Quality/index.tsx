...
export const QualityStateContext = createContext<StateContext | undefined>(
  undefined,
);
export const QualityDispatchContext = createContext<
  DispatchContext | undefined
>(undefined);

const QualityContextProvider: React.FC<{ children: ReactNode }> = ({
  children,
}) => {
  const [state, dispatch] = useReducer(reducer, initialState);
  const enhancedDispatch = useCallback(
    (action: QualityAction) => dispatch(action),
    [dispatch],
  );
